...
const ShoppingCart = () => {
  ...
  const makePayment = async () => {
    const stripe = await loadStripe('pk_test_51PL0xDRwmrKMd7zLBpbOLYaoiIYMJeGsSBhSHlbqzcznom6yzAfVwXhZrX4b0tAxELOInSm9EmLYS3rL2KQ76jMa00HBC7L8R0');
  
    // Extract necessary information from the cart state
    const data = course?.data.map((item) => ({
      title: item.course.Title,
      description: item.course.Price,
    ...
    try {
      const response = await fetch('http://localhost:5000/v1/cart/payment', {
        method: 'POST',
        headers: {
